# Урок 4. Page Object и Screen-Steps Pattern

## Цель

Познакомиться с паттерном **Page Object** и его развитием — паттерном **Steps**. Научиться разделять структуру экрана и логику взаимодействия с ним. А так же сразу начать применять фишки Kaspresso и Kakao Compose упрощающие написание тестов.

---

## Часть 1. Классический Page Object

### Зачем нужен Page Object

В предыдущих уроках вы видели, как тесты обрастают дубликатами. Выбор элемента по ID или тегу разбросан по всем тестам. Если разработчик поменяет элемент, вам придётся править его во всех файлах. Чем больше тестов — тем больше мест для исправлений и тем выше вероятность ошибки.

**Page Object** решает эту проблему. Это объект, который представляет экран (или его часть) и инкапсулирует работу с элементами.

### Основные принципы (Мартин Фаулер)

1. **Разделение ответственности.** Тест говорит **что** делать (сценарий), а Page Object знает **как** это сделать (найти кнопку, нажать).
2. **Один экран = Один класс.**
3. **Инкапсуляция локаторов.** Теги и ID скрыты внутри Page Object. Наружу торчат только понятные методы или элементы.
4. **Методы возвращают Page Object.** Это позволяет строить цепочки (`loginPage.enterName().clickLogin()`).

### Что это даёт

| Без Page Object | С Page Object |
|---|---|
| Тег `"avatar_screen_next_button"` в 15 тестах | Тег в **одном** месте — `AvatarSelectScreen.nextButton` |
| Изменение тега → правки в 15 файлах | Изменение тега → правка в **одном** файле |
| Тест: `onNodeWithTag("avatar_screen_next_button")` — что это? | Тест: `nextButton { assertIsDisplayed() }` — сразу понятно |
| Новый тестировщик: «что за строка?» | Новый тестировщик: «а, кнопка на экране аватара» |

В классическом Page Object методы объекта **не содержат assertions** (проверок). Они просто возвращают состояние или другие Page Object-ы. Проверки остаются в тесте.

---

## Часть 2. Kaspresso: ComposeScreen и KNode

### Библиотека Kakao Compose

Kaspresso использует библиотеку [Kakao Compose](https://github.com/KakaoCup/Compose) для описания Page Object-ов. Её ключевые компоненты:

- **`ComposeScreen`** — базовый класс для Page Object (аналог «экрана»)
- **`KNode`** — описание элемента внутри экрана (кнопка, поле, текст и т.д.)
- **`onComposeScreen`** — DSL-функция для работы с экраном внутри теста

### Создание ComposeScreen

Каждый экран описывается как класс, наследующий `ComposeScreen`:

```kotlin
class AvatarSelectScreen(semanticsProvider: SemanticsNodeInteractionsProvider) :
    ComposeScreen<AvatarSelectScreen>(semanticsProvider = semanticsProvider) {

    val nextButton: KNode = child {
        hasTestTag(Tags.AVATAR_NEXT_BUTTON)
    }

    fun avatar(index: Int): KNode = child {
        hasTestTag("${Tags.AVATAR_ICON}_$index")
    }
}
```

Разберём по частям:

- `semanticsProvider` — передаётся автоматически, когда вы используете `onComposeScreen`
- `ComposeScreen<AvatarSelectScreen>` — базовый класс с типовым параметром (самореференция)
- `child { ... }` — объявление элемента как **дочернего узла** экрана
- `hasTestTag(...)` — матчер для поиска узла по `testTag`

В тесте:
```kotlin
onComposeScreen<AvatarSelectScreen>(composeTestRule) {
    nextButton.performClick()
}
```

### KNode — описание элемента

`KNode` — это «виртуальный элемент» на экране. Он не ищет узел при создании, а хранит **набор матчеров** и выполняет поиск только при обращении (assertion или action).


Матчеры совпадают со [шпаргалкой](https://developer.android.com/develop/ui/compose/testing/testing-cheatsheet?hl=ru), а еще их можно комбинировать:

```kotlin
val loginButton: KNode = child {
    hasTestTag(Tags.NAME_LOGIN_BUTTON)
    hasText("Войти")
}
```

### viewBuilderAction — привязка экрана к контейнеру

Если экран имеет свой контейнер с тегом, можно указать его через `viewBuilderAction`. Тогда все `child`-элементы будут искаться **только внутри** этого контейнера:

```kotlin
class AvatarSelectScreen(semanticsProvider: SemanticsNodeInteractionsProvider) :
    ComposeScreen<AvatarSelectScreen>(
        semanticsProvider = semanticsProvider,
        viewBuilderAction = { hasTestTag(Tags.AVATAR_CONTAINER) }
    ) {
    // nextButton ищется только внутри avatar_screen_container
    val nextButton: KNode = child {
        hasTestTag(Tags.AVATAR_NEXT_BUTTON)
    }
}
```

> [!TIP]
> `viewBuilderAction` — необязательный параметр. Если экран — это абстракция без конкретного контейнера, можно его не указывать.

### KakaoComposeTestRule — экраны без boilerplate

Возможно вы заметили, что в каждом вызове `onComposeScreen` мы передаём `composeTestRule`:

```kotlin
onComposeScreen<AvatarSelectScreen>(composeTestRule) {
    nextButton.performClick()
}
```

Библиотека Kakao Compose предоставляет **`KakaoComposeTestRule`** — JUnit-правило, которое регистрирует `composeTestRule` **глобально**. После этого:

1. **`onComposeScreen` больше не требует аргумента** — правило само подставляет `semanticsProvider`
2. **Экраны можно объявлять как `object`** — не нужен конструктор с `semanticsProvider`
3. **`onComposeScreen` можно заменить на имя объекта** — `AvatarSelectScreen { ... }`

#### Screen как object

До:
```kotlin
class AvatarSelectScreen(semanticsProvider: SemanticsNodeInteractionsProvider) :
    ComposeScreen<AvatarSelectScreen>(semanticsProvider = semanticsProvider) {
    // ...
}
```

После:
```kotlin
object AvatarSelectScreen : ComposeScreen<AvatarSelectScreen>() {
    // ...
}
```

#### Как это выглядит в тесте

```kotlin
class AvatarSelectScreenTest : BaseTestCase() {

    @Test
    fun checkAvatarScreen() = run {
        step("Проверить, что отображаются 4 иконки аватаров") {
            for (i in 0..3) {
                AvatarSelectScreen.avatar(i).assertIsDisplayed()
            }
        }

        step("Проверить, что кнопка «Далее» отображается") {
            AvatarSelectScreen.nextButton.assertIsDisplayed()
        }
    }

    @Test
    fun checkAvatarError() = run {
        step("Нажать на кнопку 'Далее'") {
            AvatarSelectScreen.nextButton {
                assertIsDisplayed()
                performClick()
            }
        }

        step("Проверить отображение ошибки") {
            AvatarSelectScreen.error {
                assertIsDisplayed()
                assertTextEquals("Сначала выберите аватар!")
            }
        }
    }
}
```
--- 

## Часть 3. Паттерн Steps

На некоторых проектах часто идут дальше и разделяют **Элементы** и **Логику**.

Если мы напишем `performClick()` и `assertIsDisplayed()` прямо в классе экрана, он станет слишком большим и будет смешивать "что на экране" с "что мы с ним делаем".

Поэтому выделяют **Test Steps** (шаги теста) в отдельные объекты.

### Структура паттерна

1. **Screen (Экран)** — отвечает только за **поиск элементов**. Хранит KNode и локаторы.
2. **Steps (Шаги)** — отвечают за **действия и проверки**. Используют Screen для доступа к элементам.

> [!NOTE]
> Это лишь один из паттернов организации кода. На маленьких проектах часто пишут действия прямо в Page Object или даже в тесте. Но подход Steps делает тесты максимально читаемыми и переиспользуемыми.

### Пример реализации: AvatarSelectSteps

Создаём объект `AvatarSelectSteps`, который содержит методы-действия:

```kotlin
object AvatarSelectSteps {

    fun selectAvatar(index: Int) {
        AvatarSelectScreen {
            with(avatar(index)) {
                assertIsDisplayed()
                performClick()
            }
        }
    }

    fun clickNextBtn() {
        AvatarSelectScreen {
            nextButton {
                assertIsDisplayed()
                performClick()
            }
        }
    }

    fun assertAvatarDisplayed(index: Int) {
        AvatarSelectScreen {
            avatar(index).assertIsDisplayed()
        }
    }

    fun assertNextButtonDisplayed() {
        AvatarSelectScreen.nextButton.assertIsDisplayed()
    }

    fun assertErrorDisplayed(expectedText: String) {
        AvatarSelectScreen {
            error {
                assertIsDisplayed()
                assertTextEquals(expectedText)
            }
        }
    }
}
```

Обратите внимание, что к элементам можно обращаться как используя DSL стиль, так и точечную нотацию. Последняя удобней для одиночных проверок и позволяет сократить код до одной строки `AvatarSelectScreen.nextButton.assertIsDisplayed()`.

---

### Как выглядит тест

Теперь тест выглядит как сценарий на естественном языке, без технических деталей (`onNode`, `performClick`):

```kotlin
class AvatarSelectScreenTest : BaseTestCase() {

    @Test
    fun checkAvatarScreen() = run {
        step("Проверить, что отображаются 4 иконки аватаров") {
            for (i in 0..3) {
                AvatarSelectSteps.assertAvatarDisplayed(i)
            }
        }

        step("Проверить, что кнопка «Далее» отображается") {
            AvatarSelectSteps.assertNextButtonDisplayed()
        }
    }

    @Test
    fun checkAvatarError() = run {
        step("Нажать на кнопку 'Далее'") {
            AvatarSelectSteps.clickNextBtn()
        }

        step("Проверить отображение ошибки") {
            AvatarSelectSteps.assertErrorDisplayed("Сначала выберите аватар!")
        }
    }
}
```

---

## BaseTestCase

Чтобы `ComposeScreen` работал корректно и поддерживал интерцепторы (автопрокрутка, логирование, повтор действий), тесты должны использовать `Kaspresso.Builder.withComposeSupport()`.


Мы вынесли это в базовый класс `BaseTestCase`:

```kotlin
open class BaseTestCase : TestCase(
    kaspressoBuilder = Kaspresso.Builder.withComposeSupport()
) {

    @get:Rule
    val composeTestRule = createAndroidComposeRule<MainActivity>()

    @get:Rule
    val kakaoComposeTestRule = KakaoComposeTestRule(
        semanticsProvider = composeTestRule
    )
}
```

Теперь все тесты наследуются от `BaseTestCase`. Так же `composeTestRule` объявлен **один раз** в базовом классе, а `kakaoComposeTestRule` регистрирует его для всех экранов.


### Почему assertIsDisplayed() перед действием

Из документации Kaspresso:

> Элемент может существовать в семантическом дереве, но **физически не быть видимым** на экране. В этом случае `performClick()` сработает «в пустоту» — тест упадёт позже с непонятной ошибкой.
> 
> Вызов `assertIsDisplayed()` перед действием запускает **Kaspresso Interceptors**.

> [!IMPORTANT]
> **Всегда вызывайте `assertIsDisplayed()` перед `performClick()` и другими действиями.** Это не только проверка, но и триггер для автопрокрутки и других интерцепторов Kaspresso.

---

## Задание: Рефакторинг с использованием Screen-Steps

### Шаг 1 — Переключиться на подготовленную ветку

Мы уже реализовали паттерн Steps для первого экрана (`AvatarSelectScreen` + `AvatarSelectSteps`).

Переключитесь на ветку:
```bash
git checkout po-prepared
```

Изучите файлы:
- `screens/AvatarSelectScreen.kt` (только элементы)
- `steps/AvatarSelectSteps.kt` (действия и проверки)
- `tests/AvatarSelectScreenTest.kt` (тест на степах)

### Шаг 2 — Создайте Screen и Steps для остальных экранов

Вам нужно реализовать ту же структуру для экранов ввода имени и ленты.

1. **NameInput**
   - `screens/NameInputScreen.kt` — элементы (`nameField`, `loginButton`)
   - `steps/NameInputSteps.kt` — методы `enterName(name)`, `clickLogin()`, `assertTitle(...)`

2. **Feed**
   - `screens/FeedScreen.kt` — элементы
   - `steps/FeedSteps.kt` — методы `assertDate(...)`, `assertPost(...)`

### Шаг 3 — Перепишите LoginFlowTest через Steps

Перепишите `LoginFlowTest.kt` так, чтобы он использовал только Steps-объекты. В самом тесте не должно быть ни одного `onComposeScreen` или `onNode`.

**Примерный вид итога:**

```kotlin
step("Сценарий входа") {
    AvatarSelectSteps.selectAvatar(2)
    AvatarSelectSteps.clickNext()
    
    NameInputSteps.enterName("User")
    NameInputSteps.clickLogin()
    
    FeedSteps.checkTitle("User")
}
```

---

## 📚 Полезные ссылки

- [Page Object — Martin Fowler](https://martinfowler.com/bliki/PageObject.html)
- [Поддержка Compose в Kaspresso](https://kasperskylab.github.io/Kaspresso/ru/Wiki/Jetpack_Compose/)
- [Kakao Compose](https://github.com/KakaoCup/Compose)
